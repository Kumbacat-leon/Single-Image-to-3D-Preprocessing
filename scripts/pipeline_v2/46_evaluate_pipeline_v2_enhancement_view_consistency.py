...
STAGING_DIR = (
    CORRECTED_ROOT
    / "_pipeline_v2_enhancement_view_consistency_staging"
)

PAIRWISE_CSV = "pipeline_v2_enhancement_view_pairwise.csv"
METHOD_SUMMARY_CSV = "pipeline_v2_enhancement_view_method_summary.csv"
COMPARISON_CHANGES_CSV = (
    "pipeline_v2_enhancement_view_comparison_changes.csv"
)
COMPARISON_STATISTICS_CSV = (
    "pipeline_v2_enhancement_view_comparison_statistics.csv"
)
RANKING_CSV = "pipeline_v2_enhancement_ranking.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Baseline, Ratio 80%, and three foreground-only enhancement "
            "variants."
        )
    )
    ...
    mode.add_argument(
        "--check-only",
        action="store_true",
        help="Validate all 75 mesh files without computing metrics.",
    )

    mode.add_argument(
        "--run",
        action="store_true",
        help="Compute and publish enhancement-screening results.",
    )
    ...
    for path in (
        ...
        if path.exists():
            raise FileExistsError(
                "Pipeline V2 enhancement evaluation output already "
                f"exists and will not be overwritten: {path}"
            )
...
def run_evaluation(
    ...
    try:
        ...
        method_summary_rows = create_method_summary_rows(
            pairwise_rows
        )
    ...
    ratio_80_comparisons = [
        row
        for row in statistics_rows
        if row["group"] == "overall"
        and row["metric"] == "chamfer_distance"
        and row["from_method"] == "ratio_80"
    ]
    ...
    print(
        "PIPELINE V2 ENHANCEMENT VIEW-CONSISTENCY RESULTS"
    )
    ...
    print(
        "\nRatio-80-to-enhancement Chamfer comparisons:"
    )

    for row in ratio_80_comparisons:
        print(
            f"{row['comparison']:<38} "
    ...
    print(
        "\nBaseline-to-V2 Chamfer comparisons:"
    )
    ...
    print(
        f"\nBest pilot V2 configuration by the predefined rule: "
        f"{best['method']}"
    )
    ...
    print(
        "PIPELINE V2 ENHANCEMENT VIEW-CONSISTENCY EVALUATION PASSED."
    )

    print(
        "Note: this is development-set screening. Lock the selected "
        "configuration before evaluating the expanded final dataset."
    )
...
def main() -> None:
    ...
    print(
        "Pipeline V2 Ratio-80 Foreground Enhancement "
        "- Cross-View Consistency Evaluation"
    )
    ...
    if args.check_only:
        print(
            "\nCHECK PASSED: no Pipeline V2 enhancement "
