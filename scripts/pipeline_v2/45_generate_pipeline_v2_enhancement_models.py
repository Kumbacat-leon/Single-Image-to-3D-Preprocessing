...
STAGING_OUTPUT_ROOT = (
    PROJECT_ROOT
    / "_pipeline_v2_enhancement_outputs_staging"
)
...
RESULTS_ROOT = (
    CORRECTED_ROOT
    / "pipeline_v2_enhancement_generation"
)

STAGING_RESULTS_ROOT = (
    CORRECTED_ROOT
    / "_pipeline_v2_enhancement_generation_staging"
)
...
MANIFEST_CSV = (
    "pipeline_v2_enhancement_generation_manifest.csv"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate 45 TripoSR meshes for Pipeline V2 "
            "foreground-only enhancement screening."
        )
    )
    ...
    mode.add_argument(
        ...
        action="store_true",
        help=(
            "Validate candidate inputs and TripoSR paths "
            "without generating models."
        ),
    )
    ...
    mode.add_argument(
        ...
        action="store_true",
        help=(
            "Generate all 45 enhancement-screening meshes."
        ),
    )
...
def validate_destination_is_new() -> None:
    ...
    if existing:
        ...
        raise FileExistsError(
            "Pipeline V2 enhancement-generation output "
...
def preflight() -> dict[str, object]:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            f"Missing {len(missing)} enhancement "
            f"candidate input(s):\n{details}"
        )
...
def run_triposr(
    ...
    if output_directory.exists():
        raise FileExistsError(
            "TripoSR output directory already exists: "
            f"{output_directory}"
        )
...
def validate_completed_outputs(
    ...
    if successful != expected_count:
        raise RuntimeError(
            ...
            "enhancement-screening meshes succeeded."
        )
...
def publish() -> None:
    ...
    if RESULTS_ROOT.exists():
        raise FileExistsError(
            f"Results appeared during execution: "
            f"{RESULTS_ROOT}"
        )
...
def run_generation(
    ...
    print(
        "PIPELINE V2 ENHANCEMENT MODEL-GENERATION RESULTS"
    )
    ...
    print(
        "PIPELINE V2 ENHANCEMENT MODEL GENERATION PASSED."
    )
...
def main() -> None:
    ...
    print(
        "TripoSR settings: CUDA, chunk-size=1024, "
        "mc-resolution=96, no-remove-bg"
    )
