...
OUTPUT_DIR = CORRECTED_ROOT / "ablation_view_consistency"
STAGING_DIR = CORRECTED_ROOT / "_ablation_view_consistency_staging"

PAIRWISE_CSV = "ablation_view_consistency_pairwise.csv"
METHOD_SUMMARY_CSV = "ablation_view_consistency_method_summary.csv"
STAGE_PAIRWISE_CSV = "ablation_view_consistency_stage_changes.csv"
STAGE_STATISTICS_CSV = "ablation_view_consistency_stage_statistics.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Deterministically compare cross-view geometry consistency for "
            "Baseline, NoBG-only, NoBG+Crop/Pad, and Final Proposed."
        )
    )
    ...
    mode.add_argument(
        "--check-only",
        action="store_true",
        help="Validate the 60 mesh files without computing metrics.",
    )
    mode.add_argument(
        "--run",
        action="store_true",
        help="Compute and publish corrected ablation view consistency.",
    )
    ...
    for path in (OUTPUT_DIR, STAGING_DIR):
        if path.exists():
            raise FileExistsError(
                "Ablation view-consistency output already exists and will "
...
def preflight() -> dict[str, object]:
    ...
    for method, root in METHOD_ROOTS.items():
        method_paths[method] = validate_mesh_set(root, method)
    ...
    return {
        ...
        "expected_pairwise_rows": (
            len(METHOD_ROOTS)
            * len(OBJECTS)
            * len(tuple(combinations(VIEWS, 2)))
        ),
        "expected_stage_rows": (
            len(TRANSITIONS)
            * len(OBJECTS)
    ...
    for method in METHOD_ROOTS:
        for object_name in OBJECTS:
            ...
            for metric in METRICS:
                ...
                row[f"stdev_{metric}"] = float(
                    np.std(values, ddof=1)
                )
                row[f"minimum_{metric}"] = float(
                    np.min(values)
                )
                row[f"maximum_{metric}"] = float(
                    np.max(values)
                )
...
def create_stage_pairwise_rows(
    pairwise_rows: list[dict],
) -> list[dict]:
    indexed = {
        (
...
def create_stage_statistics_rows(
...
def validate_row_counts(
    ...
    expected_pairwise = int(
        summary["expected_pairwise_rows"]
    )
    expected_stage = int(
        summary["expected_stage_rows"]
    )
...
def run_evaluation(
    ...
    try:
        ...
        pairwise_rows = create_pairwise_rows(cache)
        method_summary_rows = (
            create_method_summary_rows(
                pairwise_rows
            )
        )
        stage_rows = create_stage_pairwise_rows(
            pairwise_rows
        )
        statistics_rows = (
            create_stage_statistics_rows(
                stage_rows
            )
        )
        ...
        write_csv(
            STAGING_DIR / STAGE_PAIRWISE_CSV,
            stage_rows,
        )
        write_csv(
            STAGING_DIR / STAGE_STATISTICS_CSV,
            statistics_rows,
        )
    ...
    overall_rows = [
        ...
        and row["metric"] == "chamfer_distance"
    ]

    print("\n" + "=" * 100)
    print("CORRECTED ABLATION VIEW-CONSISTENCY RESULTS")
    print("=" * 100)
    print(
        f"Pairwise rows: "
        f"{len(pairwise_rows)}/"
    ...
    print(
        f"Stage pair rows: "
        f"{len(stage_rows)}/"
        f"{summary['expected_stage_rows']}"
    )
    print(
        f"Stage statistical rows: "
        f"{len(statistics_rows)}/48"
    )

    print("\nOverall Chamfer-distance transitions:")
    ...
    print(f"\nSaved: {OUTPUT_DIR}")
    print(
        "CORRECTED ABLATION VIEW-CONSISTENCY EVALUATION PASSED."
    )
    print(
        "Note: view-pair rows within the same object share source meshes. "
        "Treat inferential p-values as exploratory."
    )
...
def main() -> None:
    ...
    print("=" * 100)
    print("Corrected Ablation Cross-View Consistency Evaluation")
    print("=" * 100)
    ...
    if args.check_only:
        print(
            "\nCHECK PASSED: no ablation view-consistency "
